...
//        }
    ...
    @Override
    public ResultSet viewAllProductResultSetForOrder(ProductBean productBean, int num) {
        ResultSet rst = null;
        try {
            if (num == 1) {
                PreparedStatement ps = con.prepareStatement("SELECT p.id,c.category,p.product,p.quantity,p.size,p.price,p.productsindhi "
                        + "FROM product p,category c WHERE p.category=c.id "
                        + "AND p.productsindhi LIKE '" + productBean.getProductNameSindhi() + "%'");
                rst = ps.executeQuery();
            } else {
                PreparedStatement ps = con.prepareStatement("SELECT p.id,c.category,p.product,p.quantity,p.size,p.price,p.productsindhi "
                        + "FROM product p,category c WHERE p.category=c.id "
                        + "AND p.product LIKE '" + productBean.getProductName() + "%'");
                rst = ps.executeQuery();
            }
        } catch (SQLException e) {
            e.printStackTrace();
